...
class PEARL(MetaRLAlgorithm):
    ...
    def __init__(
        ...
        encoder_spec = self.get_env_spec(self._single_env, latent_dim,
                                         'encoder', use_next_obs_in_context)
    ...
    def train(self, trainer):
        ...
        for _ in trainer.step_epochs():
            ...
            # obtain samples from random tasks & gather data for train task evaluation
            train_paths = []
            ...
            for _ in range(self._num_tasks_sample):
                ...
                if self._num_steps_posterior > 0:
                    posterior_paths = self._obtain_samples(trainer, epoch,
                ...
                if self._num_extra_rl_steps_posterior > 0:
                    extra_paths = self._obtain_samples(trainer,
                                         epoch,
    ...
    def _train_once(self, epoch):
        ...
        # Log training data only when we eval
        if epoch % self._epochs_per_eval == 0:
            with tabular.prefix('Losses'):
                ...
                tabular.record('/GlobalKLLoss', (kl_loss / self._kl_lambda).item())
                tabular.record('/GlobalKLLossWithCoef', kl_loss.item())
                tabular.record('/TotalLocalGlobalLoss', kl_loss.item())
    ...
    def _optimize_policy(self, indices):
        ...
        # optimize qf and encoder networks. As outputs of a forward pass, these all requires_grad
        ...
        if self._use_information_bottleneck:
            kl_div = self._policy.compute_kl_div() #requires_grad=True
            global_enc_mean = self._policy.z_means
            global_enc_vars = self._policy.z_vars
        ...
        return policy_loss, vf_loss, qf_loss, kl_loss, policy_mean.mean(), policy_log_std.mean(), log_pi.mean(), global_enc_mean.mean(), global_enc_vars.mean()      
    ...
    def adapt_policy(self, exploration_policy, exploration_episodes):
        ...
        if self._use_next_obs_in_context:
            ctxt = np.hstack((o, a, r, no)).reshape(1, total_steps, -1) #eg. 10 episodes*200 steps = 2000 total_steps
        else:
            ctxt = np.hstack((o, a, r)).reshape(1, total_steps, -1) #eg. 10 episodes*200 steps = 2000 total_steps
        context = torch.as_tensor(ctxt, device=global_device()).float()
        #1 task, total_steps batch size
        #therefore, context takes into account ALL OBSERVED EPISODES WHEN TESTING!!
        self._policy.infer_posterior(context) 
    ...
    @classmethod
    def get_env_spec(cls, env_spec, latent_dim, module, use_next_obs_in_context=False):
