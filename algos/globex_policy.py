...
class LocalGlobalContextualPolicy(nn.Module):
    ...
    Args: 
        ...
        policy (garage.torch.policies.Policy): Inner policy network
        
        tanh_actions_before_sampling (bool): Whether to tanh transform the mean of the distribution before sampling
        ...
        local_kl_normal_prior (bool): True if local z prior is N(0,1), False if we use local z dist
        sample_global_embedding (bool): Whether to pass global_z to the policy if True, or distribution mean/var if False
        sample_local_embedding (bool): Whether to pass local_z to the policy if True, or distribution mean/var if False
        min_std (float): Minimum std for encoder distribution
    """

    def __init__(self, latent_dim, global_encoder, local_encoder, transition_decoder, reward_decoder, state_decoder, 
                 global_recurrent_encoder, local_recurrent_encoder,
                 policy, tanh_actions_before_sampling=False, use_next_obs = True,
                 disable_local_encoder=False, disable_global_encoder=False, local_kl_normal_prior = True, 
                 sample_global_embedding=True, sample_local_embedding=True, min_std=1e-10):
        super().__init__()
    ...
    def infer_global_posterior(self, context, detach_every=None):
        ...
        Args:
            ...
            detach_every (int): truncated backprop through time param, if using a recurrent encoder
        """
        if self._global_recurrent_encoder:
    ...
    def infer_local_posterior(self, context, detach_every=None, save_prev_dist=False):
        ...
        Args:
            context (torch.Tensor): Context values, with shape
                ...
                C is the combined size of observation, action, and reward.
            detach_every (int): truncated backprop through time param, if using a recurrent encoder
            save_prev_dist (bool): Save previous distributions, used during training
        """
    ...
    def get_action(self, obs, context):
        ...
        Returns:
            ...
            dict:
                * global_z, local_z: latent z samples
                * local_context: context used to generate local_z
                * prev_local_context: context used to generate previous local_z
                * prior_reset_ind: if local_z was recently reset
        """
